...
`SourceFile` porte `translated_count: i64` et `total_count: i64` (`#[sqlx(default)]`) — calculés dans `get_source_files` par `LEFT JOIN segments` + agrégation ; les requêtes qui ne font pas ce JOIN (ex: `debug_inject_file`) utilisent `0 as total_count, 0 as translated_count` comme littéraux.
