...
impl Engine {
    /// Map a project's `engine` field (`"wolf"`, `"mv_mz"`, `"vx_ace"`, `"bakin"`, …)
    /// to the placeholder pattern set to use. Only `"wolf"` gets Wolf-specific
    /// patterns; every other engine shares the RPG Maker (MV/MZ) patterns.
    pub fn from_project_engine(engine: &str) -> Self {
        if engine == "wolf" {
            Engine::Wolf
        } else {
            Engine::MvMz
...
mod tests {
    ...
    // ---------------------------------------------------------------------------
    // Engine::from_project_engine
    // ---------------------------------------------------------------------------

    #[test]
    fn test_from_project_engine_wolf() {
        assert_eq!(Engine::from_project_engine("wolf"), Engine::Wolf);
    }

    #[test]
    fn test_from_project_engine_defaults_to_mvmz() {
